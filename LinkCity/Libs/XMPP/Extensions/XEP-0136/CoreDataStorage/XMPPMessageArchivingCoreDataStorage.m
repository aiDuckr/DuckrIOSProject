...
- (void)didInsertMessage:(XMPPMessage *)message coreDataMessage:(XMPPMessageArchiving_Message_CoreDataObject *)coreDataMessage{
    // Override hook
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationDidReceiveXMPPMessage object:nil userInfo:@{XMPPMessageKey:message}];
    });
}
                                                    ...
                                                    managedObjectContext:(NSManagedObjectContext *)moc
{
    ...
    //Roy. 2015.1.13
    // why composing==YES ?
    // remove the predicate "composing==YES" to do request, because our message stored in database have composing==NO
	//NSString *predicateFrmt = @"composing == YES AND bareJidStr == %@ AND outgoing == %@ AND streamBareJidStr == %@ AND msgUuid == %@";
    NSString *predicateFrmt = @"bareJidStr == %@ AND outgoing == %@ AND streamBareJidStr == %@ AND msgUuid == %@";
	NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFrmt,
	                             [messageJid bare], [NSNumber numberWithBool:isOutgoing], [streamJid bare], uuid];
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    //LCLogInfo(@"archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream %@,isOutgoing:%@,callStack:%@", message,isOutgoing?@"YES":@"NO",[NSThread callStackSymbols]);
    
    //Roy 2015.5.26
    //屏蔽某人后，再给这个人发消息，会返回type = error 的消息
    //防止出现在自己和自己对话的bug
    ...
    if (data) {
        ...
        if (!error && [[jsonDic allKeys]containsObject:@"MsgUuid"]) {
            NSString *infoUUID = [jsonDic objectForKey:@"MsgUuid"];
            if ([LCStringUtil isNotNullString:infoUUID]) {
                uuid = infoUUID;
            }
        }
    }
	...
	[self scheduleBlock:^{
		...
		XMPPMessageArchiving_Message_CoreDataObject *archivedMessage =
		    [self composingMessageWithJid:messageJid
		                        streamJid:myJid
                                 ...
                                 msgUuid:uuid
		             managedObjectContext:moc];
        //RLog(@"%@ - ArchivedMessage:%@",[[NSThread callStackSymbols]firstObject],archivedMessage);
		...
		else
		{
            ...
            archivedMessage.info = info;
            archivedMessage.msgUuid = uuid;
			...
			if (didCreateNewArchivedMessage) // [archivedMessage isInserted] doesn't seem to work
			{
                ...
                [self didInsertMessage:message coreDataMessage:archivedMessage];    //Override hook
			}
			else
			{
			...
			if ([messageBody length] > 0)
			{
				...
				if (didCreateNewContact) // [contact isInserted] doesn't seem to work
				{
                    ...
                    if ([message isChatMessage]) {
                        contact.contactType = [NSString stringWithFormat:@"%d", CHAT_CONTACT_TYPE_USER];
                    }else if ([message isGroupMessage]){
                        contact.contactType = [NSString stringWithFormat:@"%d", CHAT_CONTACT_TYPE_GROUPCHAT];
                    }else {
                        contact.contactType = [NSString stringWithFormat:@"%d", CHAT_CONTACT_TYPE_ALL];
                    }
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationReceiveChatMessageAddContact object:nil userInfo:@{XMPPMessageKey:message}];
                    });
				}
				else
				{
...
#pragma mark - Roy Changed
/**Roy 2015.1.13
    Override:
        to set the private managedObjectContext merge policy to NSMergeByPropertyStoreTrumpMergePolicy
 */
