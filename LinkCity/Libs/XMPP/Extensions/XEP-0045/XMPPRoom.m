...
- (void)sendMessage:(XMPPMessage *)message
{
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [message addAttributeWithName:@"to" stringValue:[roomJID full]];
//        NSLog(@"the message room jid is %@", [roomJID full]);
...
- (void)sendMessageWithBody:(NSString *)messageBody withInfo:(NSString *)info
{
    ...
    {
        [message addAttributeWithName:@"info" stringValue:info];
    }
...
{
    /*  Roy 2015.8.8 to save CPU
     because we do not use XMPP to retrive Chat Room member
     ignore presence of MUC
     */
    return;
