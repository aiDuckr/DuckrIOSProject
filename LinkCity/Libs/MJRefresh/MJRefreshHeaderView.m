...
- (void)layoutSubviews
{
    ...
    CGSize labelSize = [self.statusLabel intrinsicContentSize];
    CGSize picSize = self.arrowImage.frame.size;
    //RLog(@"%@",NSStringFromCGSize(labelSize));
    
    // 1.箭头
    CGFloat arrowX = self.mj_width*0.5 - labelSize.width*0.5 - 10*0.5;
    ...
    // 3.状态文字
    CGFloat labelX = self.mj_width*0.5 + picSize.width*0.5 + 10*0.5;
    self.statusLabel.frame = CGRectMake(0, 0, labelSize.width, labelSize.height);
    self.statusLabel.center = CGPointMake(labelX, self.mj_height*0.5);
...
//    self.statusLabel.center = CGPointMake(0, self.mj_height/2);
