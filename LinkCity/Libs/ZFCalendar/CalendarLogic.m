...
- (id)init
{
    ...
    if (self)
    {
        self.startCalendarDay = nil;
        self.endCalendarDay = nil;
    }
    return self;
}
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    //NSDateComponents *calendarSelectStart = [selectStart YMDComponents];//默认选择的那一天
...
//        calendarDay.style = CellDayTypeStartClick;
//      
//    //没被点击选中
//    }
//    else
    ...
    //===================================
    //这里来设定预设的选中日期的状态
    NSDateComponents *calendarSelectStart = nil;
    NSDateComponents *calendarSelectEnd = nil;
    if (selectStart) {
        calendarSelectStart = [selectStart YMDComponents];
    }
    if (selectEnd) {
        calendarSelectEnd = [selectEnd YMDComponents];
    }
    if (calendarSelectStart &&
        (calendarSelectStart.year == calendarDay.year &
         calendarSelectStart.month == calendarDay.month &
         calendarSelectStart.day == calendarDay.day))
    {
        if (calendarSelectEnd &&
            (calendarSelectEnd.year == calendarDay.year &
             calendarSelectEnd.month == calendarDay.month &
             calendarSelectEnd.day == calendarDay.day))
        {
            calendarDay.style = CellDayTypeStartEndClick;
            self.startCalendarDay = calendarDay;
            self.endCalendarDay = calendarDay;
        }else{
            calendarDay.style = CellDayTypeStartClick;
            self.startCalendarDay = calendarDay;
        }
    }else if (calendarSelectEnd &&
        (calendarSelectEnd.year == calendarDay.year &
         calendarSelectEnd.month == calendarDay.month &
         calendarSelectEnd.day == calendarDay.day))
    {
        calendarDay.style = CellDayTypeEndClick;
        self.endCalendarDay = calendarDay;
    }
...
- (void)resetStartOrEndDay:(CalendarDayModel *)day
{
    //周末
...
{
    if (self.optiondaynumber == 1) {
        [self selectLoginForOptionDayNumOne:day];
    }else {
        [self selectLogicForOptionDayNumTwo:day];
    }
}

- (void)selectLoginForOptionDayNumOne:(CalendarDayModel *)day{
    if (day.style == CellDayTypeStartClick) {
        if (self.startCalendarDay) {
            [self resetStartOrEndDay:self.startCalendarDay];
        }
        
        day.style = CellDayTypeStartClick;
        
        self.startCalendarDay = day;
        
        if ([self.delegate respondsToSelector:@selector(startAndEndDayFinished:)]) {
            [self.delegate startAndEndDayFinished:self];
        }
        
        return;
    }
    
    if (nil == self.startCalendarDay) {
        day.style = CellDayTypeStartClick;
        self.startCalendarDay = day;
        return;
    }
    ...
    if (nil != self.startCalendarDay) {
        ...
        day.style = CellDayTypeStartClick;
        self.startCalendarDay = day;
        [self resetStartOrEndDay:self.endCalendarDay];
        self.endCalendarDay = nil;
    }
}

- (void)selectLogicForOptionDayNumTwo:(CalendarDayModel *)day{
    if (day.style == CellDayTypeStartClick || day.style == CellDayTypeEndClick) {
        
        if (self.startCalendarDay) {
            [self resetStartOrEndDay:self.startCalendarDay];
        }
        if (self.endCalendarDay) {
            [self resetStartOrEndDay:self.endCalendarDay];
        }
        
        day.style = CellDayTypeStartEndClick;
        
        self.startCalendarDay = day;
        self.endCalendarDay = day;
        
        if (self.delegate && [self.delegate respondsToSelector:@selector(startAndEndDayFinished:)])
        {
            [self.delegate startAndEndDayFinished:self];
        }
    ...
    if (day.style == CellDayTypeStartEndClick || day.style == CellDayTypeEndClick) {
        return;
    }
    
    if (nil == self.startCalendarDay && nil == self.endCalendarDay)
    {
        day.style = CellDayTypeStartClick;
        self.startCalendarDay = day;
        return ;
    ...
    if (nil != self.startCalendarDay && nil == self.endCalendarDay)
    {
        if (1 == [[self.startCalendarDay date] compare:[day date]])
        {
            [self resetStartOrEndDay:self.startCalendarDay];
            self.startCalendarDay = nil;
            return ;
        }
        day.style = CellDayTypeEndClick;
        self.endCalendarDay = day;
        if (self.delegate && [self.delegate respondsToSelector:@selector(startAndEndDayFinished:)])
        {
            [self.delegate startAndEndDayFinished:self];
