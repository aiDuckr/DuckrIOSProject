...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.headerReferenceSize = CGSizeMake(DEVICE_WIDTH, 25.0f);//头部视图的框架大小
        
        self.itemSize = CGSizeMake(DEVICE_WIDTH/7, 58.0f);//每个cell的大小
        
        self.minimumLineSpacing = 0.0f;//每行的最小间距
        
        self.minimumInteritemSpacing = 0.0f;//每列的最小间距
        
        self.sectionInset = UIEdgeInsetsMake(0, 0, 12.0f, 0);//网格视图的/上/左/下/右,的边距
    }
