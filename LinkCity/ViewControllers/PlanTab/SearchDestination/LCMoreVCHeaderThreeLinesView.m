...
@interface LCMoreVCHeaderThreeLinesView ()<LCCalendarChooseDateViewDelegate,LCFilterTagButtonDelegate>
...
@implementation LCMoreVCHeaderThreeLinesView
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
...
+ (instancetype)createInstance {
    UINib *nib = [UINib nibWithNibName:@"LCMoreVCHeaderThreeLinesView" bundle:nil];
    NSArray *views = [nib instantiateWithOwner:nil options:nil];
    for (UIView *v in views) {
        if ([v isKindOfClass:[LCMoreVCHeaderThreeLinesView class]]) {
            v.translatesAutoresizingMaskIntoConstraints = YES;
            return (LCMoreVCHeaderThreeLinesView *)v;
        }
    }
    return nil;
}
...
- (void)updateRequestVariable {
    ...
    if ([self.delegate respondsToSelector:@selector(sendStateToVCFromThreeLinesViewWithDateArray:priceArray:orderType:timeBtnArry:)]) {
        [self.delegate sendStateToVCFromThreeLinesViewWithDateArray:self.dateArr priceArray:self.priceArr orderType:self.orderType timeBtnArry:self.timeBtnArry];
    }
}
