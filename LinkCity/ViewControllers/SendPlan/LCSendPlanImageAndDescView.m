...
#define PLACEHOLDER_IMAGE [UIImage imageNamed:@"SendPlanAddImageIcon"]
static const int Max_Pick_Image_Count = 3;


@interface LCSendPlanImageAndDescView()<UIActionSheetDelegate, UITextViewDelegate>
@property (nonatomic, strong) UIButton *justClickedButton;

@end


@implementation LCSendPlanImageAndDescView

+ (instancetype)createInstance{
    UINib *nib = [UINib nibWithNibName:NSStringFromClass([LCSendPlanImageAndDescView class]) bundle:nil];
    
    NSArray *views = [nib instantiateWithOwner:nil options:nil];
    for (UIView *v in views){
        if ([v isKindOfClass:[LCSendPlanImageAndDescView class]]) {
            v.translatesAutoresizingMaskIntoConstraints = NO;
            return (LCSendPlanImageAndDescView *)v;
        }
    }
...
- (void)updateShowWithPlan:(LCPlanModel *)plan{
    ...
    if ([LCStringUtil isNotNullString:self.plan.descriptionStr]) {
        self.descTextView.text = self.plan.descriptionStr;
    }else{
...
- (void)updateImgBtnAppearance{
    ...
    if ([self.delegate respondsToSelector:@selector(sendPlanImageAndDescViewDidUpdateImage:)]) {
        [self.delegate sendPlanImageAndDescViewDidUpdateImage:self];
    }
}
...
- (void)textViewDidEndEditing:(UITextView *)textView{
    ...
    if ([self.delegate respondsToSelector:@selector(sendPlanImageAndDescViewDidEndInputDescription:)]) {
        [self.delegate sendPlanImageAndDescViewDidEndInputDescription:self];
    }
}
                       ...
                       [self.plan.imageUnits enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                           ...
                           if ([LCStringUtil isNullString:unit.url] && unit.image) {
                               ...
                               [LCImageUtil uploadImageDataToQinu:imgData imageType:ImageCategoryPlan completion:^(NSString *imgUrl) {
                                   self.uploadingImageNum --;
                                   LCLogInfo(@"DiduploadImg uploadingNum:%ld url:%@",(long)self.uploadingImageNum,imgUrl);
...
- (void)didFinishUploadImages{
    if ([self.delegate respondsToSelector:@selector(sendPlanImageAndDescViewDidEndUploadImage:)]) {
        [self.delegate sendPlanImageAndDescViewDidEndUploadImage:self];
    }
}
