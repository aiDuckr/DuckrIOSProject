...
@interface LCUserAllOrderVC ()<UITableViewDataSource, UITableViewDelegate, LCUserOrderCellDelegate, UIAlertViewDelegate>
...
@implementation LCUserAllOrderVC

+ (instancetype)createInstance {
    return (LCUserAllOrderVC *)[LCStoryboardManager viewControllerWithFileName:SBNameUserOrder identifier:VCIDUserAllOrderVC];
}
...
- (void)requestUserAllOrderFromOrderString {
    __weak typeof(self) weakSelf = self;
    [LCNetRequester getUserAllOrder_V_FIVE:self.orderStr callBack:^(NSArray *planArray, NSArray *orderArray, NSString *orderStr, NSError *error) {
        if (error) {
            ...
            if ([LCStringUtil isNullString:self.orderStr]) {
                ...
                if (nil == weakSelf.planArray || weakSelf.planArray.count <= 0) {
                    [YSAlertUtil tipOneMessage:@"你还没有购买活动"];
                }
            } else {
...
- (void)headerRefreshing {
    self.orderStr = nil;
    [self requestUserAllOrderFromOrderString];
}
...
- (void)orderEvaluatedButtonClicked:(LCUserOrderCell *)cell {
    [[LCPopViewHelper sharedInstance] popCostPlanEvaluationView:nil withPlan:[self.planArray objectAtIndex:[self.orderTableView indexPathForCell:cell].row]];
}
