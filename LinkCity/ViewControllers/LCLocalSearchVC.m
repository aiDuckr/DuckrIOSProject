...
@interface LCLocalSearchVC ()<LCCalendarChooseDateViewDelegate, UISearchDisplayDelegate, UISearchBarDelegate, LCFilterTagButtonDelegate, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate, LCSharedFuncUtilForGrayview>
...
@property (weak, nonatomic) IBOutlet UIView *businessContainerView;

@property (weak, nonatomic) IBOutlet NSLayoutConstraint *themeContainerWidthConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *bussinessContainerWidthConstraint;
...
@property (strong, nonatomic) NSArray *businessButtonArr;
...
@implementation LCLocalSearchVC

#pragma mark - LifeCycle.
/// 首页控制器.
+ (instancetype)createInstance {
    return (LCLocalSearchVC *)[LCStoryboardManager viewControllerWithFileName:SBNameLocalTab identifier:VCIDLocalSearchVC];
}
...
- (void)commonInit {
    ...
    self.businessButtonArr = [[NSArray alloc] init];
...
- (void)viewDidLoad {
    ...
    self.contentArr = manager.homeSearchMoreActiv;//改掉！！！localSearchActiv
    ...
    /// 初始化主题筛选按钮.
    [self initThemeAndBusinessContainerView];
    /// 初始化排序按钮.
...
/// 初始化上方搜索视图.
- (void)initSearchBar {
    self.searchBar = [[LCSearchBar alloc] initWithFrame:CGRectMake(-31.0f, 0.0f, (DEVICE_WIDTH - 15.0f * 3), 44.0f)];
...
/// 初始化主题筛选按钮.
- (void)initThemeAndBusinessContainerView {
...
#warning 商圈缺！！！
    NSMutableArray *businessArr = [NSMutableArray arrayWithArray:nil];
    if (nil != businessArr && businessArr.count > 0) {
        /// 删除滚动视图内的View的宽度约束.
        [self.businessContainerView removeConstraint:self.bussinessContainerWidthConstraint];
    }
    NSMutableArray *mutArr1 = [[NSMutableArray alloc] init];
    //...
    
    self.businessButtonArr = mutArr1;
}
...
- (void)updateShow {
    ...
    /// 缓存数据.
    LCDataManager *manager = [LCDataManager sharedInstance];
    manager.localSearchActiv = self.contentArr;
...
- (void)updateShowFilteredView {
    ...
    for (NSInteger index = 0; index < self.businessButtonArr.count; ++index) {
        LCFilterTagButton *btn = (LCFilterTagButton *)[self.businessButtonArr objectAtIndex:index];
        if (YES == btn.selected) {
            [self addFilteredButton:btn];
        }
    }
...
//    [self.tableView setContentOffset:CGPointMake(self.tableView.contentOffset.x, self.tableView.contentOffset.y + 50.0f)];
...
- (void)updateRequestVariable {
    ...
    /// 选择的商圈.
    mutArr = [[NSMutableArray alloc] init];
    for (NSInteger index = 0; index < self.businessArr.count; ++index) {
        LCFilterTagButton *btn = (LCFilterTagButton *)[self.businessArr objectAtIndex:index];
        if (YES == btn.selected) {
            [mutArr addObject:[NSString stringWithFormat:@"%zd", btn.tag]];
        }
    }
    self.businessArr = mutArr;
...
/// 活动商品的Cell.
- (UITableViewCell *)configureCell:(UITableView*)tableView withIndexPath:(NSIndexPath *)indexPath {
...
- (void)didSelectedDates:(NSArray *)dates {
    ...
    self.dateArr =  [LCSharedFuncUtil addFiltedArrayToArray:self.dateArr withUnfiltedArray:mutArr];;
...
- (void)filterTagButtonSelected:(LCFilterTagButton *)button {
    ...
    if (NSNotFound != [self.businessButtonArr indexOfObject:button]) {
        [button updateShowButtons:self.businessButtonArr];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [self configureCell:tableView withIndexPath:indexPath];
