...
- (void)reloadData{
    ...
    if ([self isEmpty] && self.tipWhenEmpty) {
        [self showTip];
    }else{
        [self hideTip];
    }
...
- (BOOL)isEmpty{
    if (self.dataSource &&
        ...
        [self.dataSource respondsToSelector:@selector(collectionView:numberOfItemsInSection:)]) {
        NSInteger sectionNum = [self.dataSource numberOfSectionsInCollectionView:self];
        for (int i=0; i<sectionNum; i++) {
            NSInteger itemNum = [self.dataSource collectionView:self numberOfItemsInSection:i];
            if (itemNum>0) {
                return NO;
            }
        }
