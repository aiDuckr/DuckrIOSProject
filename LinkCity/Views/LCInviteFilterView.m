...
@interface LCInviteFilterView()<LCFilterTagButtonDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation LCInviteFilterView

+ (instancetype)createInstance {
    UINib *nib = [UINib nibWithNibName:@"LCInviteFilterView" bundle:nil];
    NSArray *views = [nib instantiateWithOwner:nil options:nil];
    for (UIView *v in views) {
        if ([v isKindOfClass:[LCInviteFilterView class]]) {
            v.translatesAutoresizingMaskIntoConstraints = NO;
            return (LCInviteFilterView *)v;
        }
    }
    return nil;
}
