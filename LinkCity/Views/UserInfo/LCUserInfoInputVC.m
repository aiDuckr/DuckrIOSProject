...
+ (instancetype)createInstance{
    return (LCUserInfoInputVC *)[LCStoryboardManager viewControllerWithFileName:SBNameUserinfo identifier:VCIDUserInfoInputVC];
}
- (void)setInputType:(UserInfoInputType)inputType forUser:(LCUserInfo *)userInfo withDelegate:(id<LCUserInfoInputVCDelegate>)delegate{
    ...
    switch (inputType) {
        ...
        case InputTypeSigh:
            ...
            RLog(@"textView did become to first:%@",can?@"YES":@"NO");
    ...
    switch (self.inputType) {
        case InputTypeCompany:
            if (self.inputTextField.text.length > CompanyMaxNum) {
                ...
                [YSAlertUtil tipOneMessage:tip onView:self.view yoffset:-50];
                return;
            }
...
- (NSString *)getTextCountStringForSignature{
    ...
    return [NSString stringWithFormat:@"%lu/%d字",[inputtedString length],SignMaxNum];
}
@end
